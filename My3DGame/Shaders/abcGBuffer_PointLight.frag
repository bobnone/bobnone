...
void main()
{
	...
	vec3 gbufferSpecular = texture(uGSpecular, gbufferCoord).xyz;
	...
	float NdotL = dot(N, L);
	float SpecularPower = pow(max(0.0, dot(R, V)), uSpecularPower);
	if (NdotL > 0)
	{
		...
		Phong = (DiffuseColor * NdotL);// + (SpecularColor * SpecularPower);
	}

	// Final color is texture color times phong light (alpha = 1)
	outColor = vec4(gbufferDiffuse * gbufferSpecular * Phong, 1.0);
}