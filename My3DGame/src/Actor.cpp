...
void Actor::computeWorldTransform()
{
	...
	worldTransform_ = Math::createScaleMatrix(scale_);
	worldTransform_ *= Math::toMatrix(rotation_);
	worldTransform_ *= Math::createTranslationMatrix(position_);
...
void Actor::rotateToNewForward(const Vector3& forward)
{
	...
	float dot = Math::dot(Vector3::UNIT_X, forward);
	...
	else
	{
		...
		Vector3 axis = Math::cross(Vector3::UNIT_X, forward);
		axis.normalize();
