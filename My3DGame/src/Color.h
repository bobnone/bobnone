#pragma once
#include <GLM/glm.hpp>
#include "Math.h"

namespace Color
{
	static const glm::vec3 Black(0.0f, 0.0f, 0.0f);
	static const vector3 White(1.0f, 1.0f, 1.0f);
	static const glm::vec3 Red(1.0f, 0.0f, 0.0f);
	static const glm::vec3 Green(0.0f, 1.0f, 0.0f);
	static const glm::vec3 Blue(0.0f, 0.0f, 1.0f);
	static const glm::vec3 Yellow(1.0f, 1.0f, 0.0f);
	static const glm::vec3 LightYellow(1.0f, 1.0f, 0.88f);
	static const glm::vec3 LightBlue(0.68f, 0.85f, 0.9f);
	static const glm::vec3 LightPink(1.0f, 0.71f, 0.76f);
	static const glm::vec3 LightGreen(0.56f, 0.93f, 0.56f);
}