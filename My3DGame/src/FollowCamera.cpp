...
void FollowCamera::update(float deltaTime)
{
	...
	Matrix4x4 view = Math::lookAt(actualPos_, target, Vector3::UNIT_Z);
	setViewMatrix(view);
}
...
void FollowCamera::setViewMatrix(const Matrix4x4& view)
{
...
{
	Vector3 playerPos = Math::normalize(owner_->position());
	Vector3 cameraPos = Math::normalize(actualPos_);
