...
bool Mesh::Load(const std::string& fileName, Renderer* renderer)
{
	...
	mSpecularPower = static_cast<float>(doc["specularPower"].GetDouble());
	...
	mVertexArray = new VertexArray(vertices.data(), numVerts, layout, indices.data(), static_cast<unsigned>(indices.size()));
	// Save the binary mesh
	SaveBinary(fileName + ".bin", vertices.data(), numVerts, layout, indices.data(), static_cast<unsigned>(indices.size()), textureNames, mBox, mRadius, mSpecularPower);
	return true;
}
void Mesh::Unload()
{
