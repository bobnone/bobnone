...
void UIScreen::drawTexture(Shader* shader, Texture* texture, const Vector2& offset, float scale, bool flipY)
{
	...
	Matrix4x4 scaleMat = Math::createScaleMatrix(static_cast<float>(texture->getWidth()) * scale, yScale, 1.0f);
	//Translate to position on screen
	Matrix4x4 transMat = Math::createTranslationMatrix(Vector3(offset.x, offset.y, 0.0f));
	//Set world transform
